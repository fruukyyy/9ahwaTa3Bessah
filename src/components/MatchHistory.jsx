...
const MatchHistory = ({ matches }) => {
  ...
  const renderRivalryCard = (rivalry, isCustom = false) => {
    ...
    return (
    <div className={`bg-primary/50 p-3 sm:p-4 rounded-lg border-2 ${isCustom ? 'border-blue-400/50' : 'border-gold-400/30'} mb-4`}>
      {isCustom && (
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="my-1 sm:my-2 flex items-center justify-center w-full">
          <div className="text-xl sm:text-2xl font-bold text-gold-400 mx-1 sm:mx-2">
            {rivalry.headToHead.player1Wins}
          </div>
          <div className="text-lg sm:text-xl text-gold-300 mx-1 sm:mx-2">-</div>
          <div className="text-xl sm:text-2xl font-bold text-gold-400 mx-1 sm:mx-2">
        ...
        {/* Match Stats */}
        <div className="mt-2 sm:mt-3 text-xs sm:text-sm text-gold-400">
...
return (
  <div className="bg-primary p-3 sm:p-4 md:p-6">
    ...
    <div className="bg-secondary p-3 sm:p-4 md:p-6 rounded-lg mb-4 sm:mb-6 border-2 border-gold-400/20 shadow-gold-sm">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 sm:mb-4">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gold-400 mb-2 sm:mb-0">
          🔥 Player Rivalries
        </h2>
        <button
          onClick={() => setShowPlayerSelector(!showPlayerSelector)}
          className="bg-gold-500/20 hover:bg-gold-500/30 text-gold-400 px-3 py-1 rounded-lg text-xs sm:text-sm transition-colors w-full sm:w-auto"
        >
      ...
      {/* Player Selector */}
      {showPlayerSelector && (
        <div className="mb-4 sm:mb-6 bg-primary/50 p-3 sm:p-4 rounded-lg border border-gold-400/30">
          <h3 className="text-base sm:text-lg font-semibold text-gold-400 mb-2 sm:mb-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 mb-3 sm:mb-4">
            {playersList.map(player => (
              <div
                ...
                className={`p-1 sm:p-2 rounded-lg cursor-pointer transition-all border-2 ${
              ...
              >
                <div className="flex items-center">
                  <img 
                    src={player.image} 
                    alt={player.name} 
                    className="w-6 sm:w-8 h-6 sm:h-8 rounded-full mr-1 sm:mr-2 border border-gold-400/50"
                  />
                  <span className="text-xs sm:text-sm text-text truncate">{player.name}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {customRivalry && (
        <div className="mb-4 sm:mb-6">
          <h3 className="text-lg sm:text-xl font-semibold text-blue-400 mb-2 sm:mb-3">Your Selected Rivalry</h3>
      ...
      {topRivalries.length > 0 && (
        <>
          <h3 className="text-lg sm:text-xl font-semibold text-gold-400 mb-2 sm:mb-3">Top Rivalries</h3>
    ...
    {/* Search Section */}
    <div className="bg-secondary p-3 sm:p-4 md:p-6 rounded-lg mb-4 sm:mb-6 border-2 border-gold-400/20 shadow-gold-sm">
      <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 sm:mb-4 text-gold-400">
        Search Matches
      </h2>
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4">
        <div className="relative flex-grow">
          ...
          <input
            ...
            className="pl-9 sm:pl-10 w-full p-2 sm:p-3 rounded-lg focus:outline-none bg-secondary border-2 border-gold-400/30 text-text caret-gold-500 text-sm sm:text-base"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
          />
        </div>
        <select
          className="p-2 sm:p-3 rounded-lg focus:outline-none bg-secondary border-2 border-gold-400/30 text-text text-sm sm:text-base"
          value={searchType}
    ...
    {filteredMatches.length === 0 ? (
      <div className="text-center py-6 sm:py-8 md:py-12 text-text-gray text-sm sm:text-base">
      ...
      <div className="space-y-2 sm:space-y-3 md:space-y-4">
        {filteredMatches.map(match => (
          ...
          >
            <div 
              className="p-2 sm:p-3 md:p-5 flex justify-between items-center cursor-pointer hover:bg-opacity-90 transition-all bg-secondary border-b-2 border-gold-400/30"
              onClick={() => toggleMatch(match.id)}
            >
              <h3 className="font-bold text-sm sm:text-base md:text-lg text-gold-400">
            ...
            {expandedMatches.includes(match.id) && (
              <div className="animate-fade-in">
                <div className={`p-2 sm:p-3 md:p-5 ${isLandscape ? 'flex flex-row' : 'flex flex-col'} justify-between items-start gap-3 sm:gap-4 md:gap-6`}>
                  <div className={`${isLandscape ? 'w-1/3' : 'w-full'} mb-3 sm:mb-0 p-2 sm:p-3 md:p-4 rounded-lg bg-primary border-2 border-gold-400/30`}>
                    <h4 className="font-semibold text-base sm:text-lg md:text-xl mb-1 sm:mb-2 md:mb-3">
                      <span className="bg-gold-500 text-primary px-2 py-1 rounded-full text-xs sm:text-sm md:text-base">
                        Team A
                      </span>
                    </h4>
                    <ul className="space-y-1 sm:space-y-2 md:space-y-3">
                      {match.teamA.players.map(playerId => {
                        ...
                        return (
                          <li key={playerId} className="hover:bg-gold-500/10 px-1 sm:px-2 py-1 rounded">
                            <Link to={`/player/${playerId}`} className="group flex items-center">
                              <img 
                                src={player.image} 
                                alt={player.name} 
                                className="w-6 sm:w-8 h-6 sm:h-8 rounded-full mr-2 sm:mr-3 border border-gold-400/50 object-cover"
                              />
                              <div>
                                <span className="text-text font-semibold text-xs sm:text-sm md:text-base block">
                                  {player.name}
                                </span>
                  ...
                  <div className={`${isLandscape ? 'w-1/3' : 'w-full'} flex flex-col items-center justify-center my-2 sm:my-3 md:my-4`}>
                    <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-gold-400">
                      {match.teamA.score} - {match.teamB.score}
                    </div>
                    <div className={`text-sm sm:text-base md:text-lg font-semibold mt-1 ${
                      match.teamA.score > match.teamB.score 
                        ? 'text-gold-400' 
                  ...
                  <div className={`${isLandscape ? 'w-1/3' : 'w-full'} p-2 sm:p-3 md:p-4 rounded-lg bg-primary border-2 border-gold-400/30`}>
                    <h4 className="font-semibold text-base sm:text-lg md:text-xl mb-1 sm:mb-2 md:mb-3">
                      <span className="bg-gold-500 text-primary px-2 py-1 rounded-full text-xs sm:text-sm md:text-base">
                        Team B
                      </span>
                ...
                {match.mvp && (
                  <div className="p-2 sm:p-3 md:p-5 border-t border-gold-400/20 bg-primary">
                    <h4 className="font-semibold text-sm sm:text-base md:text-lg mb-1 sm:mb-2 md:mb-3 text-gold-400">
                    ...
                    >
                      <img 
                        ...
                        alt="MVP" 
                        className="w-8 sm:w-10 md:w-12 h-8 sm:h-10 md:h-12 rounded-full mr-2 sm:mr-3 border-2 border-gold-400"
                      />
                      <div>
                        <span className='font-bold text-base sm:text-lg md:text-xl text-gold-400'>
                ...
                {match.notes && (
                  <div className="p-2 sm:p-3 md:p-5 border-t border-gold-400/20 bg-primary">
                    <h4 className="font-semibold text-sm sm:text-base md:text-lg mb-1 sm:mb-2 text-gold-400">
                      Match Notes
                    </h4>
                    <p className="text-text-gray text-xs sm:text-sm md:text-base">{match.notes}</p>
                  </div>
                )}
              </div>
            )}
